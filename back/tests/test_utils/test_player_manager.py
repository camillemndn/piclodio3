...
class TestPlayerManager(unittest.TestCase):
    ...
    @patch(
        "utils.player_manager.PlayerManager.start_player_task", new_callable=AsyncMock
    )
    @patch(
        "utils.player_manager.PlayerManager.auto_kill_player_task",
        new_callable=AsyncMock,
    )
    def test_player_run(
        self, mock_auto_kill_player_task, mock_player_start, mock_run_backup_file
    ):
        """
        player executed
        player is started
        ...
        with patch.object(
            PlayerManager, "is_started", return_value=True
        ) as mock_is_started:
            self.loop.run_until_complete(
                self.player_manager.main_loop(
                    url=self.url,
                    auto_stop_seconds=5,
    ...
    @patch("utils.player_manager.PlayerManager.run_backup_file", new_callable=AsyncMock)
    def test_player_run_and_failed(
        self, mock_run_backup_file, mock_auto_kill_player_task, mock_player_start
    ):
        """
        player executed
        player not started
        ...
        """
        with patch.object(
            PlayerManager, "is_started", return_value=False
        ) as mock_is_started:
            self.loop.run_until_complete(
                self.player_manager.main_loop(
                    url=self.url,
