...
class TestUpdate(APITestCase):
    ...
    def test_update_wrong_data_sent(self):
        ...
        expected = {
            "active": [ErrorDetail(string="Must be a valid boolean.", code="invalid")],
            "webradio": [
                ErrorDetail(
                    string='Invalid pk "12" - object does not exist.',
                    code="does_not_exist",
                )
            ],
        }
    ...
    def test_update_with_default_webradio_no_webradio_provided(self):
        ...
        with patch.object(
            PlayerManager, "threaded_start", return_value=True
        ) as mock_player_start:
            response = self.client.post(self.url, data, format="json")
            expected = {
                "active": True,
                "webradio": {
