...
class SchedulerManager(object, metaclass=Singleton):
    ...
    def play_web_radio(self, url, auto_stop_minutes):
        ...
        PlayerManager().async_start(
            self.loop, url, True, auto_stop_minutes, backup_file_path
        )
    ...
    def add_new_job(
        self, job_id, day_of_week_string, hour, minute, url, auto_stop_minutes
    ) -> bool:
        print(
            "add a new job with id {}, {}, {}, {}, {}, {}".format(
                job_id, day_of_week_string, hour, minute, url, auto_stop_minutes
            )
        )
        my_cron = CronTrigger(hour=hour, minute=minute, day_of_week=day_of_week_string)
        self.scheduler.add_job(
            func=self.play_web_radio,
            trigger=my_cron,
            id=str(job_id),
