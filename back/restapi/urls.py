...
alarm_clock_urlpatterns = [
    ...
    url(
        r"^(?P<pk>[0-9]+)/$", AlarmClockDetail.as_view(), name="retrieve_update_destroy"
    ),
]
...
urlpatterns = [
    url(
        r"^webradios/",
        include((web_radio_urlpatterns, "webradios"), namespace="webradios"),
    ),
    url(
        r"^alarmclocks/",
        include((alarm_clock_urlpatterns, "alarmclocks"), namespace="alarmclocks"),
    ),
